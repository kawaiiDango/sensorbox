...
float fft_real[FFT_N] __attribute__((aligned(4)));
float spectrum[FFT_N] __attribute__((aligned(4)));
float spectrum_log[LOG_RESAMPLED_SIZE] __attribute__((aligned(4)));
...
void mic_i2s_reader_task(void *parameter)
{
    ...
    if (!i2s_inited)
    {
        ...
        // Discard first few blocks, microphone may have startup time (i.e. INMP441 up to 83ms)
        for (int i = 0; i < 3; i++)
            i2s_read(I2S_PORT, &samples, SAMPLES_SHORT * sizeof(int32_t), &bytes_read, portMAX_DELAY);
    }

    while (mic_i2s_reader_task_read)
    {
        ...
        if (!fftArrayFilled)
        {
            ...
            {
                fft_real[i] = samples[i];
                // fft_imag[i] = 0;
                // spectrum[i] = 0;
            }
            DC_BLOCKER.filter(fft_real, fft_real, FFT_N);
            fftArrayFilled = true;
        }
...
void doFft()
{
    // arduinoFft.windowing(FFTWindow::Blackman_Harris, FFTDirection::Forward, true);
    // arduinoFft.dcRemoval();
    // arduinoFft.compute(FFTDirection::Forward);
    // arduinoFft.complexToMagnitude();
    
    ESP_fft *espFft = new ESP_fft(FFT_N, SAMPLE_RATE, FFT_REAL, FFT_FORWARD, fft_real, spectrum);

    espFft->hammingWindow();
    espFft->removeDC();
    espFft->execute();
    espFft->complexToMagnitude();

    // for (int i = 0; i < FFT_N; i++)
    //     fft_real[i] = fft_real[i] * 2 / FFT_N / 10000;

    float majorPeakFreq = 0;
    float majorPeakMag = 0;
    // arduinoFft.majorPeak(majorPeakFreq, majorPeakMag);
    majorPeakFreq = espFft->majorPeakFreq();
    majorPeakMag = espFft->majorPeak();

    delete espFft;

    Serial.printf("Fundamental Freq : %.2f Hz\t Mag: %.2f \n", majorPeakFreq, majorPeakMag);

    for (int i = 0; i < FFT_N / 2; i++)
    {
        // Serial.printf("%.1f -> %.4f, ", i * float(SAMPLE_RATE) / FFT_N, spectrum[i]);
        // float y_value = spectrum[i] * 128 * 2 / FFT_N / 10000;

        float y_value = MIC_OFFSET_DB + MIC_REF_DB + 20 * log10((2 * spectrum[i] / FFT_N) / MIC_REF_AMPL);
        spectrum[i] = y_value * 2; // scale it up arbritrarily by 2 and divide it later in the flux query
    }
}

void log_resample_fft(float *fft, float *resampled_fft)
{
    int i, j, bin_start_index, bin_end_index;
    float log_bins[LOG_RESAMPLED_SIZE], sum, m;
    ...
    for (i = 0; i < LOG_RESAMPLED_SIZE - 1; i++)
    {
...
audio_measurements audio_read()
{
    ...
    xTaskCreate(mic_i2s_reader_task, "Mic I2S Reader", I2S_TASK_STACK, NULL, I2S_TASK_PRI, &mic_i2s_reader_handle);
    ...
    while (xQueueReceive(samples_queue, &q, portMAX_DELAY))
    {
        ...
        if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD)
        {
            ...
            // Serial.printf("%u FreeHeap\n", ESP.getFreeHeap());

            audio_measurements am;
            am.dbA = Leq_dB;

            doFft();
            log_resample_fft(spectrum, spectrum_log);
            am.fft = spectrum_log;
